...
def handle_apply_project(project_id, form, creator_id):
    try:
        ...
        database_manager = ProjectDatabaseManager()
        database_manager.apply_to_project(project_id, creator_id, application)
    except Exception:
        db.session.rollback()
...
def handle_project_gui(project_id):
    ...
    note = ProjectNote.query.filter_by(
        project_id=project.id, author_id=current_user.id, title=None
    ).first()
    ...
    if request.method == "POST":
        if action == "add_task":
            ...
            if body:
                msg = ChatMessage(
                    project_id=project.id, author_id=current_user.id, body=body
                )
                db.session.add(msg)
                db.session.commit()
